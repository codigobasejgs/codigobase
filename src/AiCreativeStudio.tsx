  ...
  return <section className="rounded-3xl border border-purple-400/20 bg-purple-400/[0.03] p-6 shadow-[0_0_50px_rgba(168,85,247,0.08)]">
    ...
    <div className="mt-6 grid gap-4">
      {drafts.map((draft) => <div key={draft.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
        ...
        <div className="grid gap-3 md:grid-cols-5">{draft.assets?.map((asset) => <div key={asset.id} className="overflow-hidden rounded-xl border border-white/10 bg-white/5"><button type="button" onClick={() => setSelectedAsset({ draft, asset })} className="group relative block w-full text-left outline-none ring-purple-300 transition hover:border-purple-300/70 focus:ring-2" title="Expandir imagem para conferência"><img src={asset.media_url} className="h-52 w-full object-cover transition duration-300 group-hover:scale-105" /><span className="absolute left-2 top-2 rounded-full bg-black/75 px-2 py-1 text-[11px] font-bold text-white">#{asset.position + 1}</span><span className="absolute right-2 top-2 rounded-full bg-purple-400 px-2 py-1 text-[11px] font-bold text-black"><Maximize2 size={12} className="inline mr-1" />Ver maior</span></button><button type="button" onClick={() => setSelectedAsset({ draft, asset })} className="flex w-full items-center justify-center gap-2 bg-purple-400 px-3 py-2 text-xs font-bold text-black hover:bg-purple-300"><Maximize2 size={14} />Expandir para aprovar</button></div>)}</div>
