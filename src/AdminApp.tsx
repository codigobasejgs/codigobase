...
export default function AdminApp() {
  ...
  const Dashboard = () => <div className="space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-2"><div className="rounded-3xl border border-cyan-400/20 bg-cyan-400/[0.03] p-6"><h2 className="mb-4 font-bold">Mensagens por dia</h2><div className="space-y-2">{(analyticsExtra.messagesByDay || []).slice(-7).map((item: any) => <div key={item.day} className="flex items-center gap-3"><span className="w-24 text-xs text-gray-500">{item.day}</span><div className="h-3 flex-1 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-cyan-400" style={{ width: `${Math.min(100, Number(item.count || 0) * 8)}%` }} /></div><span className="w-8 text-right text-sm text-cyan-200">{item.count}</span></div>) || <p className="text-gray-500">Sem série ainda.</p>}</div></div><div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6"><h2 className="mb-4 font-bold">Próximas ações sugeridas</h2><div className="space-y-3">{ideas.slice(0, 4).map(([title, text]) => <div key={title} className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="font-bold text-white">{title}</p><p className="mt-1 text-sm text-gray-400">{text}</p></div>)}</div></div></section>
  </div>;
