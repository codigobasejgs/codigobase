...
async function sendStatus(post: any, lockId: string): Promise<PublishOutcome> {
  ...
  try {
    ...
    if (error?.name === 'AbortError' || String(error?.message || error).includes('aborted')) return { status: 'pending_confirmation', warning: `Evolution timeout after ${SEND_TIMEOUT_MS}ms; check WhatsApp before retrying to avoid duplicate status.`, duration_ms: Date.now() - started };
    throw error;
  }
}
