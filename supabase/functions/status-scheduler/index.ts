...
async function sendStatus(post: any, lockId: string): Promise<PublishOutcome> {
  ...
  try {
    ...
    if (error?.name === 'AbortError' || String(error?.message || error).includes('aborted')) return { status: 'pending_confirmation', warning: `Evolution timeout after ${SEND_TIMEOUT_MS}ms; request may have been accepted. Not retrying automatically.`, duration_ms: Date.now() - started };
    throw error;
  }
}
